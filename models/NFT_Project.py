...
from sqlalchemy import create_engine, Column, String, Text, ForeignKey, BigInteger, Boolean, TIMESTAMP,Integer
...
class Project(Base):
    ...
    floor_price=Column("floor_price_BT",float )
    buy_token_price=Column("BT_price",float)
    earn_token_name=Column("earn_token_name",String(5))
    earn_token_price=Column("ET_price",float)
    last_updated=Column("last_updated",TIMESTAMP)
    earn_rate_ET=Column("earn_rate_ET",float)
